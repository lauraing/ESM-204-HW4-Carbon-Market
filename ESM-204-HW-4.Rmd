...
A_tax_cost <- integrate(A_supply, 0, A_tax_emissions)$value
A_tax_cost
# $1282.612

# Sector B cost of tax
B_tax_cost <- integrate(B_supply, 0, B_tax_emissions)$value
B_tax_cost
# $3692.271

# Sector C cost of tax
C_tax_cost <- integrate(C_supply, 0, C_tax_emissions)$value
C_tax_cost
# $904.9294
...
A_cost_tax_new <- integrate(A_supply, 0, A_emissions_tax_new)$value + ((100-66.65079)*A_cost_tax_new)
A_cost_tax_new
# $24219.6
  
# Sector B needs to cut by 100, abates 103.0711, so buys the following:
B_cost_tax_new <- integrate(B_supply, 0, B_emissions_tax_new)$value + ((100-103.0711)*B_cost_tax_new)
B_cost_tax_new
# $-4175.505

# Sector C needs to cut by 100, abates 52.89119, so buys the following:
C_cost_tax_new <- integrate(C_supply, 0, C_emissions_tax_new)$value + ((100-52.89119)*C_cost_tax_new)
C_cost_tax_new
# $23088.4

# Sector D needs to cut by 0 (sells all), abates 77.3869, so buys the following:
D_cost_tax_new <- integrate(D_supply, 0, D_emissions_tax_new)$value + (77.3869*D_cost_tax_new)
D_cost_tax_new
# $119775.5
...
After finding the new optimal emissions quantity for each sector based on this new marginal cost of abatement, and the new cost of abatement for each sector, the new total cost of abatement of 300 tons for Country X and Y combined is ` \$r round(Total_cost_tax_new, digits=2)`.
...
```
By doing cap and trade, net emissions across the two countries stay the same, but pollution will be reduced in Country Y, and pollution will increase in Country X, because Sectors A and C will buy carbon credits from Sector D.
